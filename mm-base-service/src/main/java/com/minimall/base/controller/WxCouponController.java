...
public class WxCouponController {
    ...
    public Object receive(@LoginUser Integer userId, @RequestBody String body) {
        ...
        if(type.equals(org.linlinjava.litemall.db.util.CouponConstant.TYPE_REGISTER)){
            return ResponseUtil.fail(WxResponseCode.COUPON_RECEIVE_FAIL, "新用户优惠券自动发送");
        }
        else if(type.equals(org.linlinjava.litemall.db.util.CouponConstant.TYPE_CODE)){
            return ResponseUtil.fail(WxResponseCode.COUPON_RECEIVE_FAIL, "优惠券只能兑换");
        }
        else if(!type.equals(org.linlinjava.litemall.db.util.CouponConstant.TYPE_COMMON)){
            return ResponseUtil.fail(WxResponseCode.COUPON_RECEIVE_FAIL, "优惠券类型不支持");
        }

        // 优惠券状态，已下架或者过期不能领取
        Short status = coupon.getStatus();
        if(status.equals(org.linlinjava.litemall.db.util.CouponConstant.STATUS_OUT)){
            return ResponseUtil.fail(WxResponseCode.COUPON_EXCEED_LIMIT, "优惠券已领完");
        }
        else if(status.equals(org.linlinjava.litemall.db.util.CouponConstant.STATUS_EXPIRED)){
