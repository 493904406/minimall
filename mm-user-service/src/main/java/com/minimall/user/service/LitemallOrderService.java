...
import com.minimall.user.domain.entity.LitemallOrderExample;
import com.minimall.user.mapper.LitemallOrderMapper;
import com.minimall.user.mapper.OrderMapper;
...
public class LitemallOrderService {
    ...
    public int updateWithOptimisticLocker(LitemallOrder order) {
        LocalDateTime preUpdateTime = LocalDateTime.now();
        order.setUpdateTime(new Date());
    ...
    public List<LitemallOrder> queryUnpaid(int minutes) {
        ...
        LitemallOrderExample example = new LitemallOrderExample();
        example.or().andOrderStatusEqualTo(OrderUtil.STATUS_CREATE).andAddTimeLessThan(new Date()).andDeletedEqualTo(false);
        return litemallOrderMapper.selectByExample(example);
    }
    ...
    public List<LitemallOrder> queryUnconfirm(int days) {
        ...
        LitemallOrderExample example = new LitemallOrderExample();
        example.or().andOrderStatusEqualTo(OrderUtil.STATUS_SHIP).andShipTimeLessThan(new Date()).andDeletedEqualTo(false);
        return litemallOrderMapper.selectByExample(example);
    }
    ...
    public List<LitemallOrder> queryComment(int days) {
        ...
        LocalDateTime expired = now.minusDays(days);
        LitemallOrderExample example = new LitemallOrderExample();
        example.or().andCommentsGreaterThan((short) 0).andConfirmTimeLessThan(new Date()).andDeletedEqualTo(false);
        return litemallOrderMapper.selectByExample(example);
    }
}
